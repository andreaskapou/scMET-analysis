...
y_lab <- list(
  ...
  "rna_epsilon" = "RNA log residual overdispersion"
)
for (m in mode) {
...
for (m in mode) {
  ...
  print(gg)
  pdf(paste0(pdf_dir, "/scmet_meanvar_", m, ".pdf"),
      width = 7, height = 5, useDingbats = FALSE)
  print(gg)
  dev.off()
}
...
met_hvf <- scmet_obj$hvf$summary %>%
  as.data.table %>% .[, anno := "prom_2000_2000"] %>%
  setnames("feature_name", "ens_id")
fwrite(met_hvf, paste0(hits_dir, "/met_hvf.txt.gz"), sep = "\t", quote = FALSE)
```
...
fwrite(metrna, file = paste0(hits_dir, "metrna_variability.csv"), 
       sep = "\t", quote = FALSE)
```
...
tmp <- metrna[metrna$color == "black", ]
tmp <- tmp[sample(NROW(tmp), 4000), ]
to.plot <- rbind(metrna[metrna$color != "black", ], tmp)
...
pdf(paste0(pdf_dir, "/metrna_variability.pdf"), width = 10, height = 4, useDingbats = FALSE)
print(gg)
dev.off()
```
