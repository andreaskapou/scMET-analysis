...
Y <- read_filter_ecker_data(filename = sprintf("%s/%s.tsv.gz", io$data_parsed, anno),
                            opts = opts, anno_dt = anno_dt, sample_metadata = sample_metadata,
...
saveRDS(fit, file = sprintf("%s/%s_%s_%s_%s.rds", outdir, anno, chro, group, mode))
fwrite(df, sprintf("%s/%s_%s_%s_%s.txt.gz", outdir, anno, chro, group, mode), sep = "\t")
cat("Finished!!\n")
