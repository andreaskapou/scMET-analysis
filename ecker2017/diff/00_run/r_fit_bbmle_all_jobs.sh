...
for (( i=0; i<${annoslength}; i++ )); do
  for (( j=0; j<${groupslength}; j++ )); do
    my_command="qsub -N mle_${annos[$i]}_${groups[$j]} \
                     -o ${logPath}mle_${annos[$i]}_${groups[$j]}.out \
                     -e ${logPath}mle_${annos[$i]}_${groups[$j]}.error \
                     -v ANNO=${annos[$i]},GROUP=${groups[$j]},OUTDIR=$outdir r_fit_bbmle.sh"
    eval $my_command
  done
done
