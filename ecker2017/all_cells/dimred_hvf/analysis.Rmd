...
to.plot$model <- factor(to.plot$model, levels = c("scmet", "binomial", "gaussian", "normdispbetasign", "random"),
                          labels = c("scMET", "Binomial", "Gaussian", "Dispersion norm", "Random"))
##########
## Plot ##
##########
for (i in names(metrics)) {
  ...
  theme(
    ...
    legend.position = c(0.89, 0.8), #"right",
  ...
  print(p)
  pdf(sprintf("%s/ecker_cluster_rev1_%s.pdf", out_dir, i), width = 8, height = 3.3)
  print(p)
  dev.off()
}
```
