...
for (cpg in opts$N_cpgs) {
  ...
  dt <- dt %>% split(., by = "cells") %>%
    ...
    rbindlist
  if (cpg == 15) {
    gg_overdisp[[iter]] <- shrinkage_overdisp_plot(dt, x_lab = "True overdispersion (CpG poor, average 10 CpGs)")
  } else {
...
for (cpg in opts$N_cpgs) {
  ...
  dt <- dt %>% split(., by = "cells") %>%
    ...
    rbindlist
  if (cpg == 15) {
    gg_mean[[iter]] <- shrinkage_mean_plots(dt, x_lab = "True mean methylation (CpG poor, average 10 CpGs)")
  } else {
