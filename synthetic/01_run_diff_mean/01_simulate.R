      ...
      for (cpg in N_cpgs) {
        for (f in N_feat) {
          ...
          sim_dt <- scmet_simulate_diff(N_feat = f, N_cells = c, N_cpgs = cpg, L = L,
                                        diff_feat_prcg_mu = diff_feat_prcg_mu,
                                        diff_feat_prcg_gamma = 0, OR_change_mu = or,
        ...
        ##------------------------
        saveRDS(object = sim_dt, file = paste0(rep_dir, "/data_ORmu", or, "_feat", f,
